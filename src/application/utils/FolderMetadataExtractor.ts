...
async function tryParseRootJs(dir: string): Promise<Omit<FolderMetadata, 'rawFolderName'> | null> {
  ...
  const { category, product, tags } = parseInfoName(infoName)
  const executionDate = data.info.startTime
    ? new Date(parseInt(data.info.startTime))
    : new Date()

  return { category, product, tags, executionDate }
}
 ...
 */
function parseInfoName(infoName: string): { category: string; product: string; tags: string[] } {
