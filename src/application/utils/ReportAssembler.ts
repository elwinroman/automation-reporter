...
export function assembleReport(
  ...
  const products = buildProducts(executions);
...
function buildProducts(executions: TestExecution[]): ProductSummary[] {
  ...
  for (const exec of executions) {
    ...
    if (!entry) {
      entry = { category: exec.metadata.category, passed: 0, total: 0, runs: [] };
      map.set(key, entry);
    }
